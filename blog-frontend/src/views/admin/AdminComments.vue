<template>
  <div class="cyber-card rounded-2xl shadow-lg p-8 animate-fade-in-up">
    ...
    <div class="bg-gray-900/50 rounded-xl border border-indigo-500/20 overflow-hidden">
        ...
        <tbody class="divide-y divide-indigo-500/10">
          <tr v-for="comment in comments" :key="comment.id" class="hover:bg-indigo-500/10 transition-colors duration-200">
            ...
            <td class="px-6 py-5 whitespace-nowrap">
              <span v-if="comment.status === 1" class="px-3 py-1 inline-flex text-xs font-mono rounded bg-emerald-900/40 text-emerald-400 border border-emerald-500/30 shadow-[inset_0_0_10px_rgba(16,185,129,0.2)]">
                APPROVED
              </span>
              <span v-else class="px-3 py-1 inline-flex text-xs font-mono rounded bg-amber-900/40 text-amber-400 border border-amber-500/30 shadow-[inset_0_0_10px_rgba(245,158,11,0.2)]">
                PENDING
              </span>
            </td>
      ...
      <div v-if="comments.length === 0" class="text-center py-16 text-indigo-300/50 font-mono tracking-widest">
        NO_COMMENTS_FOUND
      </div>
    </div>
  </div>
...
const fetchComments = async () => {
  try {
    ...
    comments.value = res // 因为 Axios 拦截器已经解包了 res.data，所以这里直接赋值
  } catch (error) {
